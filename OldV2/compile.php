...
function compileFromFile( $content='' ) {
	...
	if (false === file_put_contents($temprep.'/tmp.zip', base64_decode($content))) {
		return new SoapFault('Server', "can't create tmp.zip file !");
	}
	$zip = new ZipArchive;
	if ( $zip->open($temprep.'/tmp.zip') === true ) {
		...
		return new SoapFault('Server', "can't open zip file !");
	}
	// on se place dans le répertoire de travail tempo privé
	chdir($temprep);
	...
	if ( count($tex_file) != 1 ) {
		return new SoapFault('Server', "no or more than 1 TeX file found !");
	}
	...
	if ( runTex($bin, $main_tex_file) ) {
		...
		if ( is_file($main_tex_file.'.pdf') ) {
			return base64_encode(file_get_contents($main_tex_file.'.pdf'));
		} else {
			return new SoapFault('Server', "Could not find pdf compiled file !");
		}
		exit;
	} else {
		return new SoapFault('Server', $bin." execution error !");
